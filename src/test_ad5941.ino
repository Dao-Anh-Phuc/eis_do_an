...
//ASV / 8#-200?700/10|25$50|-500$120000!
#include <Arduino.h>
#include "ad5940.h"
#include "RampTest.h"
...
void AD5940RampStructInit(float S_Vol, float E_Vol, int StepNumber, float scanRate_mVs = 100.0f)
{
  ...
  // ? CRITICAL: Tính duration d?a trên scan rate
...
void AD5940ImpedanceStructInit(float S_Freq, float E_Freq, int numPoints, BoolFlag logEn)
{
  ...
  pImpedanceCfg->RcalVal = 223.0; // 200.0 
...
static int32_t AD5940_DPV_PlatformCfg(void)
{
  ...
  fifo_cfg.FIFOSrc = FIFOSRC_SINC3;
  fifo_cfg.FIFOThresh = 4;    // nên là b?i s? c?a 2 khi raw, nhýng DPV ð? “collapse” c?p -> v?n OK.
  AD5940_FIFOCfg(&fifo_cfg);
                           ...
                           uint32_t Step_mV, uint32_t PulseAmp_mV, uint32_t PulseWidth_ms)
{
  ...
  pDPV->FifoThresh   = 4; // tãng lên 4 cho ð?ng b? các mode khác
  pDPV->SysClkFreq   = 16000000.0f;
  pDPV->LFOSCClkFreq = LFOSCFreq;

  // Rx path
  pDPV->LPTIARtiaSel = LPTIARTIA_4K; // ð?m b?o ðúng ph?n c?ng
  ...
  pDPV->StepNumber    = 0;          // ð? thý vi?n t? tính t? range/step
  pDPV->bDACCodeInc  = (E_Vol >= S_Vol) ? bTRUE : bFALSE;  // hý?ng tãng/gi?m
  pDPV->bRampOneDir  = bTRUE;                              // quét 1 chi?u gi?ng SWV b?n ðang dùng

  // Vzero (bias) – gi? 1.3 V nhý b?n dùng ? SWV/CV
  ...
  pDPV->PrePulseWait_ms   = (float)PulseWidth_ms;  // set = t_pulse cho d? canh th?i
  pDPV->HoldAfterPulse_ms = 1.0f;                  // nh? ð? sang bý?c k? ti?p nhanh
  pDPV->bPulsePositive    = bTRUE;                 // xung dýõng; ð?i FALSE n?u c?n kh?
  ...
  // In ra toàn b? struct DPV sau khi kh?i t?o
...
void AD5940_DPV_Main(float DPV_Dep_V, float DPV_Dep_T)
{
  ...
  if (DPV_Dep_T > 0 && DPV_Dep_V >= 0) {
    Serial.printf("[DPV] Error: Deposition potential must be negative, got %.1f mV\n", DPV_Dep_V);
    return;
  }

  // ===== CH? M?T L?N PLATFORM CONFIG =====
  Serial.println("[DPV] Single PlatformCfg for both phases...");
  AD5940_DPV_PlatformCfg();  // Ch? dùng DPV platform config
  Serial.println("[DPV] PlatformCfg done");

  // ===== DEPOSITION PHASE (n?u c?n) =====
  if(DPV_Dep_T > 0) {
    Serial.printf("[DPV] Deposition start: Edep=%.1f mV, tdep=%.0f ms\n", DPV_Dep_V, DPV_Dep_T);
    
    // C?u h?nh chronoamperometry KHÔNG reset platform
    ...
    delay((uint32_t)DPV_Dep_T);
    AppCHRONOAMPCtrl(CHRONOAMPCTRL_STOPSYNC, 0);
    Serial.println("[DPV] Deposition done. Quiet time before stripping...");
    
    // Let capacitive transients decay before stripping scan
    AD5940_WakeUp(10);
    AD5940_INTCClrFlag(AFEINTSRC_ALLINT);
    delay(500);
  }
...
// T?o hàm chronoamp init KHÔNG reset platform
void AD5940AMPStructInit_DC_No_Platform(float Bias_Vol, float Time_Interval)
{
  ...
  pAMPCfg->AmpODR = 0.1f;  // Keep chrono timing stable; deposition time is controlled by outer delay.
...
void SetAMP_Duration_WakeupTimer(uint32_t duration_ms) {
  // C?u h?nh AD5940 wakeup timer cho ðúng duration_ms
...
void AD5940_ASV_Main(float depV, int depT) {
  // If deposition is disabled by either depV=0 or depT=0, run plain DPV.
  if (depV == 0.0f || depT == 0) {
    Serial.println("DPASV: deposition disabled (depV=0 or depT=0) -> run DPV only");
    AD5940_DPV_Main(0, 0);
    return;
  }

  // DPASV = deposition (negative potential) + DPV stripping
  if (depV > 0) {
    Serial.printf("DPASV Error: depV=%.1f must be <= 0\n", depV);
    return;
  }
  if (depT < 0) {
    Serial.printf("DPASV Error: depT=%d must be >= 0\n", depT);
    return;
  }

  Serial.printf("DPASV Start: S=%.1fmV, E=%.1fmV, step=%lumV, pulseAmp=%lumV, pulseWidth=%lums, dep=%.1fmV/%dms\n",
                S_Vol, E_Vol, DPV_Step_mV, DPV_PulseAmp_mV, DPV_PulseWidth_ms, depV, depT);

  // Reuse existing DPV engine with preconcentration
  AD5940_DPV_Main(depV, depT);

  Serial.println("DPASV finished");
}
 ...
 ******************************************************************************/
void setup() {
  Serial.begin(115200);               // B?t ð?u giao ti?p Serial v?i t?c ð? 115200
  inputString.reserve(200);           // D? ph?ng b? nh? cho chu?i nh?p
  Serial.println("MCU initialization successful.");
  uint32_t checkInitMCU = AD5940_MCUResourceInit(0); // Hàm kh?i t?o tài nguyên MCU
  if(checkInitMCU == 0) {
    Serial.println("MCU initialization successful.");  // In ra n?u kh?i t?o thành công
  } else {
    Serial.println("MCU initialization failed!");      // Có th? thêm d?ng này ð? x? l? l?i
  }
}
...
void loop() {
  while (Serial.available()) {
    ...
    if (inChar != '!') {
      ...
      // ? SPECIAL CV PARSING (method '1') - Ð? RIÊNG
      if(inputString[0] == '1') {
        ...
        // ? Call CV v?i scan rate
        AD5940_CV_Main(scanRate_mVs);
        
        inputString = "";
        ESP.restart();
      }
      // ? GI? NGUYÊN parsing cho các method khác (2-7)
        ...
        if (inputString[0] == '2') {
          ...
          if(inputString.indexOf("|-") > 0) {
            ...
            // Parse basic DPV parameters trý?c
            S_Vol = inputString.substring((moc1 + 1), moc2).toDouble() * 1.0;
            E_Vol = inputString.substring((moc2 + 1), moc3).toInt() * 1.0;
            DPV_Step_mV = inputString.substring((moc3 + 1), moc4).toInt();
            DPV_PulseAmp_mV = inputString.substring((moc4 + 1), moc5).toInt();
            ...
            int dep_start = inputString.indexOf("|-") + 2;  // Sau d?u |-
      ...
      // ? GI? NGUYÊN ASV parsing (method '8')
      else if(inputString[0] == '8') {
        // Format:
        // 8#S?E/step|pulseAmp$pulseWidth|depV$depT!
        // Example:
        // 8#-1000?100/5|50$40|-1100$120000!

        Serial.println("DPASV: Parsing command...");
        Serial.println("Input: " + inputString);

        // Use local delimiters (do not rely on moc5 because there are 2 '$')
        int hashPos   = inputString.indexOf('#');
        int qPos      = inputString.indexOf('?');
        int slashPos  = inputString.indexOf('/');
        int pipe1Pos  = inputString.indexOf('|', slashPos + 1);
        int dollar1Pos= inputString.indexOf('$', pipe1Pos + 1);
        int pipe2Pos  = inputString.indexOf('|', dollar1Pos + 1);
        int dollar2Pos= inputString.indexOf('$', pipe2Pos + 1);

        if (hashPos < 0 || qPos < 0 || slashPos < 0 || pipe1Pos < 0 || dollar1Pos < 0 || pipe2Pos < 0 || dollar2Pos < 0) {
          Serial.println("DPASV Error: invalid command format");
          inputString = "";
          return;
        }

        // Parse DPASV fields
        S_Vol            = inputString.substring(hashPos + 1, qPos).toFloat();
        E_Vol            = inputString.substring(qPos + 1, slashPos).toFloat();
        DPV_Step_mV      = inputString.substring(slashPos + 1, pipe1Pos).toInt();
        DPV_PulseAmp_mV  = inputString.substring(pipe1Pos + 1, dollar1Pos).toInt();
        DPV_PulseWidth_ms= inputString.substring(dollar1Pos + 1, pipe2Pos).toInt();
        DPV_Dep_V        = inputString.substring(pipe2Pos + 1, dollar2Pos).toFloat();
        DPV_Dep_T        = inputString.substring(dollar2Pos + 1).toFloat();

        if (DPV_Step_mV == 0 || DPV_PulseAmp_mV == 0 || DPV_PulseWidth_ms == 0) {
          Serial.println("DPASV Error: step/pulse params must be > 0");
          inputString = "";
          return;
        }

        // If one deposition parameter is zero, force plain DPV mode.
        if (DPV_Dep_V == 0.0f || DPV_Dep_T == 0.0f) {
          Serial.println("DPASV: depV=0 or depT=0 -> run DPV only");
          DPV_Dep_V = 0.0f;
          DPV_Dep_T = 0.0f;
        } else {
          if (DPV_Dep_V > 0 || DPV_Dep_T < 0) {
            Serial.println("DPASV Error: with deposition, depV must be <= 0 and depT must be >= 0");
            inputString = "";
            return;
          }
        }

        Serial.printf("DPASV Parsed: S=%.1f E=%.1f step=%lu pulseAmp=%lu pulseWidth=%lu dep=%.1f depT=%.0f\n",
                      S_Vol, E_Vol, DPV_Step_mV, DPV_PulseAmp_mV, DPV_PulseWidth_ms, DPV_Dep_V, DPV_Dep_T);

        AD5940_ASV_Main(DPV_Dep_V, (int)DPV_Dep_T);

        inputString = "";
        // Do NOT restart here; restarting drops COM and can break PC read thread
      }
      else {
