  ...
  case APPCTRL_START:
    {
      ...
      */
      ...
      //wupt_cfg.SeqxWakeupTime[SEQID_0] = (uint32_t)(AppSWVCfg.LFOSCClkFreq * (24*1000 / 1 - AppSWVCfg.SampleDelay) / 1000.0f) - 4 - 2;
...
static AD5940Err AppSWVSeqInitGen(void)
{
  ...
  //Cấu hình bộ DSP để nhận và nhận dữ liệu từ bộ khuếch đại (TIA) và DAC
  //Cấu hìn bộ lọc SINC3 và SINC2 giúp giảm nhiễu và cải thiện chất lượng tín hiệu
  ...
  //Lấy chuỗi lệnh đã tạo ra
  error = AD5940_SEQGenFetchSeq(&pSeqCmd, &SeqLen);
  if(error == AD5940ERR_OK)
  {
    //khởi tạo và cấu hình thông tin chuỗi lệnh
    AD5940_StructInit(&AppSWVCfg.InitSeqInfo, sizeof(AppSWVCfg.InitSeqInfo));
    if(SeqLen >= AppSWVCfg.MaxSeqLen)
      return AD5940ERR_SEQLEN;
    //Lưu trữ thông tin chuỗi l
    AppSWVCfg.InitSeqInfo.SeqId = SEQID_3; //ID chuỗi lệnh
    AppSWVCfg.InitSeqInfo.SeqRamAddr = AppSWVCfg.SeqStartAddr;//địa chỉ SRAM 
    AppSWVCfg.InitSeqInfo.pSeqCmd = pSeqCmd;//con trỏ đến chuỗi lệnh đã lấy từ bước trước
    AppSWVCfg.InitSeqInfo.SeqLen = SeqLen; //độ dài lệnh
    AppSWVCfg.InitSeqInfo.WriteSRAM = bTRUE;//báo hiệu đã ghi vào SRAM
    AD5940_SEQInfoCfg(&AppSWVCfg.InitSeqInfo);//cấu hình thông tin chuỗi lệnh
  }
  else
    return error; /* Error */
  return AD5940ERR_OK;
...
static AD5940Err AppSWVSeqADCCtrlGen(void)
{
  ...
  uint32_t WaitClks;//số chu kì cần chờ sau khi bắt đầu ADC
  ClksCalInfo_Type clks_cal; //cấu trúc chứa thông tin tính toán
  //cấu hình thông số ADC
  ...
  //Bắt đầu chuỗi lệnh và điều khiển ADC
  AD5940_SEQGenCtrl(bTRUE);
...
static AD5940Err RampDacRegUpdate(uint32_t *pDACData)
{
  ...
  { 
    ...
    case SWV_STATE0: /* Begin of Ramp  */
      ...
      //AppSWVCfg.bDACCodeInc = bFALSE;//bFALSE: tăng điện áp
...
{
    // Khi sóng vuông ở mức cao, giá trị DAC phải tăng để tạo xung dương
...
{
    // Khi sóng vuông ở mức thấp, giá trị DAC phải giảm
    if(AppSWVCfg.bDACCodeInc)
      AppSWVCfg.CurrRampCode -= (AppSWVCfg.DACCodePerStep - AppSWVCfg.DACCodePerRamp);
    else
      AppSWVCfg.CurrRampCode -= AppSWVCfg.DACCodePerStep;
...
AD5940Err AppSWVInit(uint32_t *pBuffer, uint32_t BufferSize)
{
    ...
    (AppSWVCfg.bParaChanged == bTRUE))
  {
    ...
    //   Thiết lập các giá trị ban đầu cho bộ đệm chuỗi lệnh, con trỏ thanh ghi
